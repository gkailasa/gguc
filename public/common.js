...
const CONFIG = {
  ...
  API_URL:          _isNetlify ? '/api'      : 'https://ggucapi.giri-kailasam.workers.dev/',

  // Admin worker (search + updatePayment): set URL once deployed
  ADMIN_API_URL:    _isNetlify ? '/adminapi' : 'https://ggucadmin.giri-kailasam.workers.dev/',
...
function paymentNoteHtml(amount, eventKey, flat, hideStatusLink) {
  ...
  const waMsg    = encodeURIComponent(
    `Hi, I have completed the payment for ${evName} registration${flatPart}. Please find the payment screenshot attached.`
  );
  ...
  <div class="next-step">
    ...
    <div class="step-content">
      <div class="step-label">Pay <strong>&#8377;${amount}</strong> via UPI</div>
      <button onclick="openPayModal(${amount}, '${eventKey}', ${flatArg})" class="step-pay-btn">Pay Now &rarr;</button>
    </div>
  </div>
  <div class="next-step">
  ...
  ${hideStatusLink ? '' : '<a href="status.html" target="_blank" class="check-status-link">Check your registration status &rarr;</a>'}
</div>`;
}
